...
public class ScopeBuilder extends SyntacticScope {
    ...
    protected final List<ScopeBuilder> myChildren = new LinkedList<ScopeBuilder>();
    ...
    FinalizedScope seal(MathSymbolTable finalTable) {
        return new FinalizedScope(finalTable, myDefiningElement, myRootModule, myParent, myBindings);
    }

    public ProgramVariableEntry addProgramVariable(String name, ResolveConceptualElement definingElement,
            PTType type) throws DuplicateSymbolException {

        sanityCheckBindArguments(name, definingElement, type);

        ProgramVariableEntry entry = new ProgramVariableEntry(name, definingElement, myRootModule, type);
    ...
    public FacilityEntry addFacility(FacilityDec facility) throws DuplicateSymbolException {

        SymbolTableEntry curLocalEntry = myBindings.get(facility.getName().getName());
        if (curLocalEntry != null) {
            throw new DuplicateSymbolException(curLocalEntry);
        }

        FacilityEntry entry = new FacilityEntry(facility, myRootModule, getSourceRepository());
    ...
    public OperationEntry addOperation(String name, ResolveConceptualElement definingElement,
            List<ProgramParameterEntry> params, PTType returnType) throws DuplicateSymbolException {

        sanityCheckBindArguments(name, definingElement, returnType);

        OperationEntry entry = new OperationEntry(name, definingElement, myRootModule, returnType, params);
    ...
    public ProcedureEntry addProcedure(String name, ResolveConceptualElement definingElement,
            OperationEntry correspondingOperation) throws DuplicateSymbolException {

        sanityCheckBindArguments(name, definingElement, "");

        ProcedureEntry entry =
                new ProcedureEntry(name, definingElement, myRootModule, correspondingOperation);
    ...
    public RepresentationTypeEntry addRepresentationTypeEntry(String name, RepresentationDec definingElement,
            ProgramTypeDefinitionEntry definition, PTType representationType, Exp convention,
            Exp correspondence) throws DuplicateSymbolException {

        sanityCheckBindArguments(name, definingElement, "");

        RepresentationTypeEntry result =
                new RepresentationTypeEntry(myTypeGraph, name, definingElement, myRootModule, definition,
                        new PTRepresentation(myTypeGraph, representationType, definition), convention,
                        correspondence);
    ...
    public ProgramTypeEntry addProgramTypeDefinition(String name, TypeDec definingElement, MTType model,
        ...
        Exp finalizationRequires = (finalization == null) ? null : finalization.getRequires();
        Exp finalizationEnsures = (finalization == null) ? null : finalization.getEnsures();

        ProgramTypeEntry entry =
                new ProgramTypeDefinitionEntry(myTypeGraph, name, definingElement, myRootModule, model,
                        new PTFamily(model, name, exemplarSymbol.getName(), definingElement.getConstraint(),
                                initRequires, initEnsures, finalizationRequires, finalizationEnsures),
                        exemplarEntry);
    ...
    public ProgramParameterEntry addFormalParameter(String name, ResolveConceptualElement definingElement,
            ParameterMode mode, PTType type) throws DuplicateSymbolException {

        sanityCheckBindArguments(name, definingElement, type);

        ProgramParameterEntry entry =
                new ProgramParameterEntry(myTypeGraph, name, definingElement, myRootModule, type, mode);
     ...
     *             imported module.
     */
    public MathSymbolEntry addBinding(String name, SymbolTableEntry.Quantification q,
        ...
        MathSymbolEntry entry =
                new MathSymbolEntry(myTypeGraph, name, q, definingElement, type, typeValue, schematicTypes,
    ...
    public MathSymbolEntry addBinding(String name, SymbolTableEntry.Quantification q,
            ResolveConceptualElement definingElement, MTType type) throws DuplicateSymbolException {
        return addBinding(name, q, definingElement, type, null, null, null);
    }

    public MathSymbolEntry addBinding(String name, ResolveConceptualElement definingElement, MTType type,
            MTType typeValue) throws DuplicateSymbolException {

        return addBinding(name, SymbolTableEntry.Quantification.NONE, definingElement, type, typeValue, null,
                null);
    }
    ...
    private void sanityCheckBindArguments(String name, ResolveConceptualElement definingElement, Object type)
        ...
        if (type == null) {
            throw new IllegalArgumentException("Symbol table entry type must " + "be non-null.");
        }
    }
}
