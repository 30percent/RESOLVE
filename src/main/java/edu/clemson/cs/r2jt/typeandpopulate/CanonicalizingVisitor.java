...
public class CanonicalizingVisitor extends MutatingVisitor {
    ...
    private final Map<String, String> myCanonicalToEnvironmentOriginal = new HashMap<String, String>();

    private final List<TypeRelationshipPredicate> myPredicates = new LinkedList<TypeRelationshipPredicate>();
    ...
    public void mutateBeginMTType(MTType t) {
        if (mySpentFlag) {
            throw new IllegalStateException("Cannot reuse a " + this.getClass() + ".  Make a new one.");
        }
    ...
    public void mutateEndMTType(MTType t) {
        if (atRoot()) {
            ...
            Map<String, MTType> quantifiedVariables = new HashMap<String, MTType>();
            ...
            myFinalExpression = new MTBigUnion(myTypeGraph, quantifiedVariables, myFinalExpression);
    ...
    public void beginMTNamed(MTNamed t) {
        ...
        try {
            ...
            try {
                ...
                MathSymbolEntry entry =
                        (MathSymbolEntry) myEnvironment.queryForOne(new UnqualifiedNameQuery(t.name));

                if (entry.getQuantification().equals(SymbolTableEntry.Quantification.UNIVERSAL)) {
                    originalBinding = entry.getType();
                    myCanonicalToEnvironmentOriginal.put(canonicalName + myPredicateSuffix, t.name);
                }
            }
        ...
        if (originalBinding != null) {
            ...
            MTNamed canonicalTypeWithSuffix = new MTNamed(myTypeGraph, canonicalName + myPredicateSuffix);
            ...
            myPredicates.add(new IsInPredicate(myTypeGraph, canonicalTypeWithSuffix, originalBinding));
            ...
            MTNamed lastReplace = (MTNamed) getInnermostBindingAnnotation(t.name, "LastReplace");

            if (lastReplace != null) {
                myPredicates.add(new EqualsPredicate(myTypeGraph, lastReplace, canonicalTypeWithSuffix));
            }
            annotateInnermostBinding(t.name, "LastReplace", canonicalTypeWithSuffix);
    ...
    public void annotateInnermostBinding(String name, Object key, Object value) {
        ...
        try {
            ...
            try {
                SymbolTableEntry entry = myEnvironment.queryForOne(new UnqualifiedNameQuery(name));

                Map<Object, Object> annotations = myEnvironmentAnnotations.get(entry);
                if (annotations == null) {
    ...
    public void boundEndMTBigUnion(MTBigUnion t) {
        ...
        for (String var : t.getQuantifiedVariables().keySet()) {
            Object lastReplace = getInnermostBindingAnnotation(var, "LastReplace");
            ...
            if (lastReplace == null) {
                ...
                throw new IllegalArgumentException("Universal type variable " + "\"" + var
                        + "\" is not concretely bound.");
            }
        }
